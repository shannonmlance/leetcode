...
class Solution:
    def twoSum(self, numbers, target):
        ...
        while i < j:
            ...
            # if the current sum is equal to the target sum, return the indexes
            if current == target:
                return [i+1, j+1]
            # or, if the current sum is greater than the target sum, decrement the end pointer
            elif current > target:
                j -= 1
            # or, if the current sum is less than the target sum, increment the beginning pointer
            else:
                i += 1
        return
